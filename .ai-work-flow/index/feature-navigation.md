...
| AI 路由网关（后端领域） | AI Routing Gateway、SQLite、Keychain、账号池、OAuth、额度、模型、价格 | 暂无（当前阶段不注册入口） | `src-tauri/src/ai_routing_gateway/`、`src-tauri/src/shared_sqlite/` | 独立于 Protocol Router；已提供安全存储、账号/组/标签、模型映射、动态额度和价格快照领域；第三方 Codex OAuth 因无公开官方契约保持发布门禁；不启动服务 |
