...
| AI 路由网关（后端领域） | AI Routing Gateway、SQLite、Keychain、账号池、OAuth、额度、模型、价格、Gateway Key、OpenAI Responses、Chat Completions、请求日志、费用、每日聚合、保留维护 | 暂无（当前阶段不注册入口） | `src-tauri/src/ai_routing_gateway/`、`src-tauri/src/shared_sqlite/`；HTTP 入口定义于 `src-tauri/src/ai_routing_gateway/runtime.rs`，日志/统计/维护入口定义于 `src-tauri/src/ai_routing_gateway/request_logs.rs`，前向 schema 位于 `schema_v1.sql`/`schema_v2.sql`/`schema_v3.sql` | 独立于 Protocol Router；提供安全存储、账号/组/标签、模型映射、动态额度、价格快照、网关 Key、确定性路由、脱敏请求/attempt 日志、不可计算费用、本机日期每日聚合及固定 `127.0.0.1:17688` 四端点 runtime；第三方 Codex OAuth 因无公开官方契约保持发布门禁；当前阶段不接应用生命周期自动启动 |
