...
| AI 环境 | AI 环境、CLI、Claude、Codex、Gemini、OpenCode | `src/components/AiEnvironments/index.tsx` | `src-tauri/src/ai_env/commands.rs`、`src-tauri/src/app_store/service_provider_commands.rs`（`service_provider_read_opencode_config`） | 环境预设/导入/激活；OpenCode 卡片点击按 `provider_key` 只读刷新单个运行时 provider 配置，不修改 canonical state 或激活状态 |
