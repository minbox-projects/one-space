...
| AI 路由网关 | AI Routing Gateway、SQLite、Keychain、账号池、OAuth、额度、模型、价格、Gateway Key、OpenAI Responses、Chat Completions、请求日志、费用、每日聚合、保留维护 | `src/components/AiRoutingGateway/`、`src/lib/aiRoutingGateway.ts`、`src/App.tsx`，稳定导航 ID 为 `ai-routing-gateway` | `src-tauri/src/ai_routing_gateway/commands/`、`src-tauri/src/ai_routing_gateway/`、`src-tauri/src/shared_sqlite/`、`src-tauri/src/app_runtime/run_app.rs`；HTTP 入口定义于 `runtime.rs`，日志/统计/维护入口定义于 `request_logs.rs` | 独立侧边栏功能模块，位于 AI 用量后、Skills 前；不属于 More Tools 或 Launcher 工具；五页签覆盖首页、账号池、网关密钥、请求日志和设置；typed facade 集中全部 `ai_routing_gateway_*` invoke 与四类可释放事件；`run_app.rs` 托管独立 state、全部 IPC、自动启动和有界退出排空，设置保存驱动受控 stop/rebind；第三方 Codex OAuth 因无公开官方契约保持发布门禁 |
