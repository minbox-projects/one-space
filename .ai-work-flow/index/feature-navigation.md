...
| AI 路由网关（安全存储基座） | AI Routing Gateway、SQLite、Keychain、凭据加密 | 暂无（当前阶段不注册入口） | `src-tauri/src/ai_routing_gateway/`、`src-tauri/src/shared_sqlite/` | 独立于 Protocol Router；固定数据库 `~/.config/onespace/data/onespace.sqlite3`，当前仅提供完整 schema、迁移和安全状态，不启动服务 |
