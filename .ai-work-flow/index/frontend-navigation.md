...
| AI 路由网关 | `src/components/AiRoutingGateway/`、`src/lib/aiRoutingGateway.ts` | `src/App.tsx` 直接挂载并传递 `isVisible`；账号详情维护独立 API Key 连接、同名模型映射与账号价格覆盖，OAuth 仅展示；密钥通过专用 IPC 复制完整值并即时保存多分组；顶栏仅在网关实际运行时显示状态图标；稳定 ID `ai-routing-gateway` 仅作为独立侧边栏模块 |
