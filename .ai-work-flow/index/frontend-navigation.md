...
| `ai-work-flow` | `src/components/AiWorkFlowTool.tsx` | `src/components/MoreToolsHub.tsx` 的工具卡片；`src/lib/navigation.ts` 解析至 `more-tools` 详情，`src/App.tsx` 保持选中与返回状态 |
...
| AI 环境 | `src/components/AiEnvironments/`、`src/lib/serviceProviders.ts` | `src/components/AiEnvironments/index.tsx`；OpenCode 服务商详情在卡片点击时按 `provider_key` 只读加载运行时配置，失败保留 canonical 缓存详情并显示错误；`src/components/AiEnvironments/opencodeModelConfig.ts` 负责 OpenCode 配置解析、校验、模型合并与序列化，`src/components/AiEnvironments/OpenCodeModelForm.tsx` 是动态模型表单入口 |
...
| AI 路由网关 | `src/components/AiRoutingGateway/`、`src/components/AiRoutingGateway/KeyManagement.tsx`、`src/lib/aiRoutingGateway.ts` | `src/App.tsx` 直接挂载并传递 `isVisible`；`KeyManagement.tsx` 是当前密钥分组、表格、一次性明文和转换工作流入口；`aiRoutingGateway.ts` 提供密钥转换 typed facade，提交仅含 key/tool/activate；账号详情维护独立 API Key 连接、同名模型映射与账号价格覆盖，OAuth 仅展示；顶栏仅在网关实际运行时显示状态图标；稳定 ID `ai-routing-gateway` 仅作为独立侧边栏模块 |
...
| AI Work Flow | `src/components/AiWorkFlowTool.tsx`、`src/lib/aiWorkFlow.ts` | `AiWorkFlowTool` 提供受管安装/更新、取消、阶段日志和环境 CRUD/use/status；`aiWorkFlow` facade 仅封装固定 Tauri invoke 命令，不读取 README 或 OneSpace AI Environments |
...
| AI Work Flow facade | `src/lib/aiWorkFlow.ts`（`aiWorkFlow*` typed invoke 包装） |
