...
- [x] 执行 OneSpace 全量自动检查、完整 Rust 与 `installed=true`/`version=null` 前后端定向回归、`git diff --check` 和应用管理仓库内 AI Work Flow 四项验收命令；在真实桌面与移动浏览器视口运行完整前端，仅替换 Tauri invoke，完成截图与布局检查。真实 Tauri GUI、网络安装更新、真实进程/文件副作用及人工端到端场景作为非阻塞延期验证记录，不作为任务完成门槛。
...
- [x] OneSpace 的 `npm run test`、`npm run lint`、`npm run build`、完整 Rust 测试、`installed=true`/`version=null` 前后端定向回归和 `git diff --check` 均通过；受管理 AI Work Flow 仓库的 `npm test`、`npm run validate:skills`、`node agent-build/install.mjs validate`、`node agent-build/install.mjs env status` 均通过。
- [x] 完整前端在仅替换 Tauri invoke 的真实桌面和移动浏览器视口中运行，截图与布局检查确认页面非空白、无重叠或水平溢出，关键入口和状态可见；测试桩只证明 invoke 契约、状态与 UI 渲染，不宣称覆盖真实 Tauri IPC、clone/pull、外部进程取消或真实文件副作用。
...
- [x] 在真实桌面和移动浏览器视口运行完整前端，仅将 Tauri invoke 替换为测试桩；截取安装状态、日志、环境列表与 JSON 编辑等关键状态，预期页面非空白、无重叠或水平溢出，入口、按钮、状态和错误均可见。该测试桩不得宣称覆盖真实 Tauri IPC、clone/pull、外部进程取消或真实文件副作用。
