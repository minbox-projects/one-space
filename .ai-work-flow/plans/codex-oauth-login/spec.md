...
本次修订解决任务 04 的阻断项 `standards-index-sync-001`：新增 OAuth Tauri commands、运行时注册和 TypeScript typed facade 后，现有导航索引仍将 OAuth 描述为仅展示且未注册 IPC。任务 04 采用 exhaustive write scope，必须获得所需索引的独占写权限，以在不越界的前提下完成导航同步；任务 06 仅独立复验最终索引与代码的一致性。
...
- OAuth 导航索引必须准确、可审计，且不得记录 access token、refresh token、id_token、authorization code、PKCE verifier 或完整 callback URL。
...
规划修订范围还包含任务 04 对 `.ai-work-flow/index/feature-navigation.md`、`.ai-work-flow/index/backend-navigation.md` 和 `.ai-work-flow/index/frontend-navigation.md` 的导航同步，以及任务 06 对这三份索引的只读一致性复验。任务 01-03、任务 05、既有依赖图和 OAuth 产品行为不在本次修订范围内。
...
13. 任务 04 的 exhaustive write scope 包含 `.ai-work-flow/index/feature-navigation.md`、`.ai-work-flow/index/backend-navigation.md` 和 `.ai-work-flow/index/frontend-navigation.md`，并由任务 04 独占这三份索引的写权限。
14. 任务 04 同步后的导航索引准确记录四个 OAuth commands、`src-tauri/src/app_runtime/run_app.rs` 注册入口、`src-tauri/src/ai_routing_gateway/oauth.rs` 领域入口、`src/lib/aiRoutingGateway.ts` typed facade 与 OAuth 事件订阅，且不包含敏感凭据、授权码、PKCE verifier 或完整 callback URL。
15. 任务 06 的验收和验证步骤独立检查三份导航索引与最终代码一致；其 write scope 不包含任何索引路径，且不与任务 04 的索引写范围重叠。
16. 任务数量、task_id、order、blocked_by 和既有产品验收保持不变；任务 01-03 的已完成事实和任务 04 的现有提交不得被重做、丢弃或重写。
...
- 既有 Git 实施事实必须保留：任务 01-03 已整合至 `integration`（发现时 HEAD 为 `47d2d32e29e942f87ca35e2c38716dc1c82b33a3`）；任务 04 的提交 `83e0a36dfe7509cf51379a5d6e1e589ef6509cc9` 尚未整合，后续实施须基于这些事实定位。
...
- 任务 04 的后续实施将在现有提交基础上修复全部四个 blocking findings。

## 修订记录

- `REV-001`（revision 1）：任务 04 写入三份导航索引并获得相应 exhaustive write scope；任务 06 仅复验索引一致性。保持 split 模式、任务结构、依赖关系及既有 OAuth 产品行为不变。
