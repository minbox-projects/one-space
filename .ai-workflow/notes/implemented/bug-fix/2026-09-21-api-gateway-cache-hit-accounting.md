...
- `api_gateway_usage_stats` keeps its command name and existing fields; the five additive fields are always serialized at every level, and `UsageStats` carries a nested `totals` mirror of the same values.
