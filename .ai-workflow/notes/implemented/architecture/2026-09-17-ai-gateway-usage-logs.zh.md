...
- 金额在记录时固化，因此之后修改价格只影响新请求；只有 2xx 上游响应计入 tokens，非 2xx 响应记 `failure` 且四档 token 为 0；未定价模型显示 `—`、不计入合计，并仅在请求已到达上游模型但没有匹配价格行时计入未定价请求提示。
