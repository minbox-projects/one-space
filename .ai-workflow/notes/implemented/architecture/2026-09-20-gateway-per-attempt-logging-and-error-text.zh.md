...
- 统计与分组视图只计入终止行，而不分组列表、其 `total`、状态/模型筛选与范围内 `local_model` 面考虑全部行；耗尽的非流式终止行记录最后一次观测到的上游状态（网络失败记 0），无候选行保持 502，调用方可见的响应字节不变。
