...
- Existing installations keep their configuration and usage history through the one-release in-place rename; each file is renamed only when its current name is absent, so a partially migrated directory converges with the current file winning and the old file left untouched.
