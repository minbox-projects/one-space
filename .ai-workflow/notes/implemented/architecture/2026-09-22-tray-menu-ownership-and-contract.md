...
- Accelerator hints follow the registered-shortcut semantics: a single valid key (`F5`, `Space`, `A`, `1`) or a modifier combination with a known key name is shown, `Meta` is not a modifier, and unparsable or unnamed values are omitted. When native item creation rejects an item's accelerator, `applyTrayMenu` retries the item without it, so one bad accelerator cannot drop the whole menu.
- `src/App.tsx` hosts the controller: it seeds state from window visibility, gateway and router status, tunnel counts, sharing status and the configured shortcuts, rebuilds the menu on `main-window-visibility-changed`, `api-gateway-status-update`, `protocol-router-status-update`, `ssh-tunnels-updated`, `file-sharing-updated`, `tray-shortcuts-updated` (emitted by Settings after saving shortcuts, so the hints refresh without a restart) and i18n `languageChanged`, and implements every tray action through the existing command wrappers.
...
- Menu structure, labels, enablement and check marks change in one TypeScript file plus `src/i18n.ts`, so adding or renaming a destination needs no Rust change; the Show/Hide and Quick AI Session accelerator hints are omitted when the configured shortcut value is not a valid registered-shortcut hint.
...
- Service-state actions (gateway, router, SSH batches, stop-sharing, sync) surface failures through the existing toast channel, and the gateway, router, tunnel and sharing actions re-query their state afterwards, so a failed or partially failed result never renders as fully successful; one-shot window, navigation and dialog actions do not have that treatment.
- The partial SSH batch toast is operation-neutral (`tray.action.sshBatchPartial` reports succeeded and failed counts without naming the operation), so one message serves both Connect All and Disconnect All.
