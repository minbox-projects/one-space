...
- 模板不再发布目录与价格：在首次同步或手工编辑之前，内置模板的模型清单为空，两个内置模板指向各自的模型清单 URL。被移除字段在读取时忽略；本次变更前写入的持久化模板状态仍可加载，其模型标识、显示名、协议与启用标志继续可用；旧 `api_gateway.json` 无需迁移，因为 `ProviderTemplateState` 与 `ProviderTemplateModel` 保持 serde 默认。
