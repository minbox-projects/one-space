...
持久化的网关配置携带 `GATEWAY_CONFIG_SCHEMA_VERSION = 1`（`src-tauri/src/ai_gateway/types_config.rs` 的 `GatewayConfig.schema_version`，`#[serde(default)]`，缺失 = 0 = 遗留），用量数据库带有自己的 `PRAGMA user_version` 门控。`src-tauri/src/ai_gateway/migration.rs` 是永久且版本门控的模块：其原先的一版删除契约被版本门控取代，并继续作为非测试代码中遗留文件名、遗留网关标记与遗留字段名的唯一所在。
...
- `migrate_legacy_config` 执行原 JSON 迁移：按与原先逐次读取归一化完全相同的规则把全局价格行落到各服务商、把单数 `off_peak` 折叠进 `off_peaks`，并清除服务商级运行键。
...
- 遗留配置在 schema 版本 1 上被迁移并改写一次，遗留用量数据库在 `user_version = 1` 上被清理一次；当前版本的安装读取与打开时不发生迁移写入与删除。
