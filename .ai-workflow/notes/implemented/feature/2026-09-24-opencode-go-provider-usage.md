...
- Successful snapshots are reused for five minutes only while provider ID, key and base URL remain eligible; explicit refresh bypasses reuse, and failed fetch or parsing results are not cached.
...
- Supersession: none. This adds the independent OpenCode Go usage display; the active CommandCode quota note remains accurate and unchanged.
