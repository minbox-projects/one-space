use crate::config::{self, SubagentSourceConfig, StorageConfig};
...
fn save_local_subagents_state(mut state: SubagentsLocalState) -> Result<SubagentsLocalState, String> {
    state.revision = state.revision.saturating_add(1);
...
fn subagent_matches_repository(subagent: &SubagentRecord, repo: &RepositoryRecord) -> bool {
    let same_repo_key = make_repo_key(&subagent.source_id, &subagent.source_rel_path) == repo.repo_key;
...
fn build_repository_views(
    ...
    let mut out = shared_state
        ...
        .iter()
        .filter_map(|repo| {
            let installed = build_repo_install_state(&local_state.subagents, repo);
            let installed_any =
...
fn ensure_model_dir_name_available(
    ...
    if has_dir_name_conflict(state, model, dir_name, ignore_subagent_id) {
    ...
    if dest.exists() {
        if let Some(subagent_id) = ignore_subagent_id {
            if let Some(existing) = state
                .subagents
                .iter()
                .find(|s| s.model == model && s.id == subagent_id)
            {
...
fn replace_source_entry_atomic(src_entry: &Path, dst: &Path) -> Result<(), String> {
    ...
    let src_md =
        source_entry_markdown_path(src_entry).ok_or("subagents/invalid_subagent_dir".to_string())?;
    let parent = dst.parent().ok_or("invalid destination")?;
    fs::create_dir_all(parent).map_err(|e| e.to_string())?;
...
fn source_subagent_abs_path(source: &SubagentSourceConfig, rel_path: &str) -> Result<PathBuf, String> {
...
fn read_repository_subagent_markdown(repo: &RepositoryRecord, cfg: &StorageConfig) -> Option<String> {
...
fn hydrate_local_records_from_catalog(state: &mut SubagentsLocalState, sync_state: &SubagentsSyncState) {
            ...
            let matches = candidates
                ...
                .filter(|item| {
                    subagent.models.is_empty()
                        ...
                        || item.models.iter().any(|model| subagent.models.contains(model))
                })
                .collect::<Vec<_>>();
    ...
    for item in &sync_state.catalog {
        ...
        if let Some(src_cfg) = source {
            if let Ok(source_path) = source_subagent_abs_path(src_cfg, &item.rel_path) {
                if source_entry_exists(&source_path) {
                    let dir_name =
                        read_required_subagent_dir_name_from_entry(&source_path).unwrap_or_else(|_| item.id.clone());
...
pub async fn subagents_config_save(
...
) -> Result<ApiOk<SubagentsConfigPayload>, String> {
    {
        ...
        cfg.subagents_sync_interval_minutes = Some(config_payload.subagents_sync_interval_minutes.max(5));
        cfg.subagents_new_badge_hours = Some(config_payload.subagents_new_badge_hours.clamp(1, 720));
...
#[tauri::command]
pub fn subagents_list_installed(model: Option<String>) -> Result<ApiOk<Vec<SubagentRecord>>, String> {
...
#[tauri::command]
pub fn subagents_list_catalog(model: Option<String>) -> Result<ApiOk<Vec<CatalogSubagent>>, String> {
...
#[tauri::command]
pub async fn subagents_sync_now(app: tauri::AppHandle) -> Result<ApiOk<SubagentsSyncState>, String> {
    tauri::async_runtime::spawn_blocking(move || subagents_sync_now_blocking(app))
        .await
        .map_err(|e| e.to_string())?
}

fn subagents_sync_now_blocking(app: tauri::AppHandle) -> Result<ApiOk<SubagentsSyncState>, String> {
    ...
    for source in &cfg.subagents_sources {
        ...
        let mut commit = None;
        let mut indexed: Vec<CatalogSubagent> = vec![];
        while retry < 5 {
            let sync_one = || -> Result<(String, Vec<CatalogSubagent>), String> {
                ...
                // If commit is unchanged, we usually reuse prior catalog.
                // But when prior catalog is empty (often from old/buggy parser),
                // force a rescan to allow recovery without requiring new commits.
                if prev_commit.as_deref() == Some(current_commit.as_str())
                    && !previous_source_catalog.is_empty()
                {
                    Ok((current_commit, previous_source_catalog))
                } else {
    ...
    trigger_storage_sync(app, "subagents_sync_now");
...
#[tauri::command]
pub fn subagents_repo_list_with_update() -> Result<ApiOk<Vec<RepositorySubagentView>>, String> {
    let shared_state = load_subagents_state()?;
    let local_state = load_local_subagents_state()?;
...
pub async fn subagents_repo_set_model(
    ...
    if input.enabled {
        ...
        remove_existing_record_dir_if_moved(&local_state, &input.model, &repo.subagent_id, &dest)?;
        let src = repo_storage_dir(&repo.repo_key)?;
        replace_dir_atomic(&src, &dest)?;
        let local_hash = hash_dir(&dest)?;
        local_state
            .subagents
            .retain(|s| !(s.model == input.model && s.id == repo.subagent_id));
...
pub async fn subagents_repo_delete(
    ...
    let in_use = local_state.subagents.iter().any(|s| {
        make_repo_key(&s.source_id, &s.source_rel_path) == input.repo_key
            || repo.as_ref().map(|r| s.id == r.subagent_id).unwrap_or(false)
    });
    if in_use {
...
#[tauri::command]
pub fn subagents_local_scan(input: LocalScanInput) -> Result<ApiOk<Vec<LocalSubagentCandidate>>, String> {
...
#[tauri::command]
pub async fn subagents_repo_import_folder(
    app: tauri::AppHandle,
    ...
    let _job = match acquire_job_key(dedupe_key)? {
        Some(v) => v,
        None => {
            return Err("subagents/import_busy".to_string());
        }
    };
    let _guard = job_lock().lock().map_err(|e| e.to_string())?;
    ...
    shared_state = save_subagents_state(shared_state)?;
    trigger_storage_sync(app, "subagents_repo_import_folder");
...
#[tauri::command]
pub async fn subagents_local_import(
    app: tauri::AppHandle,
    ...
    if models.is_empty() {
        return Err("subagents/models_required".to_string());
    }
    if input.selections.is_empty() {
        return Err("subagents/selections_required".to_string());
    }

    let candidates = scan_local_candidates(&root_can)?;
    let mut candidate_map: HashMap<String, LocalSubagentCandidate> = HashMap::new();
    for c in candidates {
    ...
    for selection in &input.selections {
        ...
        let Some(candidate) = candidate_map.get(&selection.rel_path) else {
            for model in &models {
                result.failed.push(LocalImportFailed {
                    ...
                    subagent_id: None,
                    model: model.clone(),
                    reason: "subagent_not_found".to_string(),
                });
            }
            continue;
        };
        ...
        if !src.join("AGENT.md").exists() {
            for model in &models {
                result.failed.push(LocalImportFailed {
                    ...
                    subagent_id: Some(candidate.subagent_id.clone()),
                    model: model.clone(),
                    reason: "subagents/invalid_subagent_dir".to_string(),
                });
            }
            continue;
        }
        let candidate_dir_name = match read_required_subagent_dir_name(&src) {
            Ok(name) => name,
            Err(err) => {
                for model in &models {
                    result.failed.push(LocalImportFailed {
        ...
        if !repo_exists {
            result.repo_added.push(LocalImportRepoAdded {
                ...
                subagent_id: repo_record.subagent_id.clone(),
        ...
        for model in &models {
            ...
            let existing_same_id = local_state
                .subagents
                .iter()
                .any(|s| s.model == *model && s.id == candidate.subagent_id);
            if strategy == "skip" && existing_same_id {
                result.skipped.push(LocalImportSkipped {
                    rel_path: candidate.rel_path.clone(),
                    subagent_id: candidate.subagent_id.clone(),
                    model: model.clone(),
            ...
            if let Err(err) = ensure_model_dir_name_available(
                ...
                model,
                &candidate_dir_name,
                Some(candidate.subagent_id.as_str()),
            ) {
                result.failed.push(LocalImportFailed {
                    rel_path: candidate.rel_path.clone(),
                    subagent_id: Some(candidate.subagent_id.clone()),
            ...
            if let Err(err) =
                remove_existing_record_dir_if_moved(&local_state, model, &candidate.subagent_id, &dest)
            {
                result.failed.push(LocalImportFailed {
                    rel_path: candidate.rel_path.clone(),
                    subagent_id: Some(candidate.subagent_id.clone()),
            ...
            local_state
                .subagents
                .retain(|s| !(s.model == *model && s.id == candidate.subagent_id));
            let record = SubagentRecord {
                id: candidate.subagent_id.clone(),
    ...
    if shared_changed {
        trigger_storage_sync(app, "subagents_local_import");
    }
    api_ok(result, combined_revision(&shared_state, &local_state))
}
...
pub async fn subagents_install(
    ...
    let dedupe_key = format!("install:{}:{}", input.model, input.subagent_ref);
    let _job = match acquire_job_key(dedupe_key)? {
        Some(v) => v,
        None => {
    ...
    let model_root = model_dir(&input.model)?;
    ensure_within(&model_root, &dest)?;
    remove_existing_record_dir_if_moved(&local_state, &input.model, &repo_record.subagent_id, &dest)?;
    let repo_src = repo_storage_dir(&repo_record.repo_key)?;
    ...
    local_state
        .subagents
        .retain(|s| !(s.model == input.model && s.id == repo_record.subagent_id));
...
pub async fn subagents_uninstall(
    ...
    let dedupe_key = format!("uninstall:{}:{}", input.model, input.subagent_id);
    let _job = match acquire_job_key(dedupe_key)? {
        Some(v) => v,
        None => {
            let state = load_local_subagents_state()?;
    ...
    if let Some(record) = state
        .subagents
        .iter()
        .find(|s| s.model == input.model && s.id == input.subagent_id)
        .cloned()
    {
    ...
    state
        .subagents
        .retain(|s| !(s.model == input.model && s.id == input.subagent_id));
            ...
            if let Some(source) = get_source(&cfg, &repo.source_id) {
                if let Ok(remote_path) = source_subagent_abs_path(source, &repo.source_rel_path) {
                    if source_entry_exists(&remote_path) {
                        markdown = read_markdown_from_source_entry(&remote_path).unwrap_or_default();
...
pub async fn subagents_repo_reload_apply(
    ...
    if should_sync_to_models {
        ...
        for model in MODELS {
            ...
            let dest = model_root.join(&repo_dir_name);
            ensure_within(&model_root, &dest)?;
            remove_existing_record_dir_if_moved(&local_state, model, ignore_subagent_id, &dest)?;
...
pub fn subagents_update_check(input: SubagentKeyInput) -> Result<ApiOk<bool>, String> {
    ...
    let mut changed = false;
    for s in &mut state.subagents {
        if s.model == input.model && s.id == input.subagent_id {
...
pub async fn subagents_update_apply(
    ...
    let dedupe_key = format!("update:{}:{}", input.model, input.subagent_id);
    let _job = match acquire_job_key(dedupe_key)? {
        Some(v) => v,
        None => {
            let state = load_local_subagents_state()?;
    ...
    let idx = state
        .subagents
        .iter()
        .position(|s| s.model == input.model && s.id == input.subagent_id)
        .ok_or("subagent not found")?;
...
pub async fn subagents_reconcile(
    ...
    let _guard = job_lock().lock().map_err(|e| e.to_string())?;
    reconcile_internal(model.as_deref()).map_err(|_| "subagents/mirror_apply_failed".to_string())?;
    let state = load_local_subagents_state()?;
    api_ok(true, state.revision)
}
...
pub fn subagents_reconcile_for_tool(tool: &str) -> Result<(), String> {
    ...
    let _guard = job_lock().lock().map_err(|e| e.to_string())?;
    reconcile_internal(Some(tool)).map_err(|_| "subagents/mirror_apply_failed".to_string())
}
...
mod tests {
    ...
    fn hash_dir_ignores_file_mtime_when_content_is_unchanged() {
        let unique = format!("onespace-subagents-hash-{}-{}", std::process::id(), now_ts());
...
"#;
    ...
    fn replace_source_entry_atomic_materializes_markdown_file() {
        let unique = format!("onespace-subagents-file-entry-{}-{}", std::process::id(), now_ts());
        let root = std::env::temp_dir().join(unique);
...
"#;
    ...
    fn scan_source_catalog_includes_markdown_file_entries() {
        let unique = format!("onespace-subagents-scan-md-{}-{}", std::process::id(), now_ts());
