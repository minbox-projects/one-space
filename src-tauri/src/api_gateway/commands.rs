...
/// The supported terminal tools whose managed API Gateway provider record
...
pub(in crate::api_gateway) fn previously_synced_terminal_tools(
    config: &GatewayConfig,
    providers_data: &serde_json::Value,
) -> Vec<String> {
...
pub(in crate::api_gateway) async fn apply_template_sync_with_terminal_refresh<F, Fut>(
