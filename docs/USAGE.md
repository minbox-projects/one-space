...
1. 打开左侧 `API 融合`，在 `本地服务` 卡片查看运行状态、端口与本地 Api 地址
...
- 每个上游维护名称、`Api 地址`、`ApiKey`、默认模型、模型映射与 `接口协议`；每条「本地模型 → 远端模型」映射都可单独选择协议，默认「跟随服务商协议」（保存为继承而非固定值），因此在同一条上游记录内就能让不同模型走不同 endpoint
- 模型解析先按请求模型匹配映射：命中映射但该映射的协议与请求不一致时，该上游不参与本次请求，也不会回退默认模型；没有匹配到映射时才回退默认模型，并要求服务商协议与请求一致，两者都不可用时该上游不参与本次请求
- `接口协议` 决定该上游接收哪种请求：`Chat Completions (/chat/completions)` 或 `Responses (/responses)`。本地服务同时接受 `/chat/completions` 与 `/responses`（也接受不带 `/v1` 的写法），但只会把请求交给协议一致的上游；协议不一致的上游不参与本次请求，且不会做请求体转换，请让调用方协议与该上游协议保持一致
...
1. 在 `终端同步` 中勾选目标，列表只显示 `tool` 为 `opencode` 或 `codex` 的终端记录
2. 点击 `一键配置所选` 写入，或点击 `同步所选` 更新
3. 写入内容是本地 Api 地址与当前默认本地 Key

说明：

- 写入只替换终端记录的 `base_url` 与 `api_key`，名称、模型、图标与启用状态等其他字段保持原值
- 不读写 `claude`、`antigravity` 记录，也不改写 `Protocol Router` 的 route 数据
- 没有启用本地 Key 时，配置与同步会被拒绝并提示先新增并启用一个本地 Key
- 如需撤销，在 `AI Environments` 中手动改回对应终端的地址与 Key

同步时机：

- 每个目标显示 `已同步` 或 `待同步`；待同步来自本地同步台账，当默认 Key 或本地 Api 地址与上次写入的不一致时即为待同步
- 修改端口或切换默认 Key 后，之前配置过的目标会重新显示为 `待同步`，需要再执行一次同步
- `同步所选` 可一次处理多个已勾选目标
