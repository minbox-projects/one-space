...
- 三张卡片分别显示当前范围的 `Tokens`、`请求数` 与 `花费（$）`；只有 2xx 上游响应计入 tokens 与花费，非 2xx 错误响应记为 `failure`、四档 token 均为 0 且不产生金额贡献；合计只累加已定价请求，未定价请求数量以提示条单独说明，`未定价` 与 `—` 只适用于已到达上游模型但没有匹配价格行的请求，未到达上游的失败按 0 成本处理、不计入未定价；取消的工具 transport 不产生业务日志，也不进入统计
