...
`API Fusion` 是把多个上游服务商聚合成一个本地入口的转发服务，默认监听 `127.0.0.1:17688`，并提供独立的上游服务商、本地 Key 与终端同步管理。入口在 `More Tools -> API Fusion`，也会出现在 `Launcher` 的内部工具中。

启用服务：

1. 打开 `More Tools -> API Fusion`，在 `本地服务` 卡片查看运行状态、端口与本地 Api 地址
...
- 每个上游维护名称、`Api 地址`、`ApiKey`、默认模型与模型映射；模型解析先按请求模型匹配映射，没有匹配时回退默认模型，两者都不可用时该上游不参与本次请求
