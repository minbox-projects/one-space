...
| 工具 | 版本 | 会话数据中的版本字段 |
|------|------|---------------------|
| **Codex** | `codex-cli 0.114.0` | ❌ 无版本字段 |
| **Claude** | `2.1.76` | ❌ 无版本字段 |
| **Gemini** | `0.33.1` | ❌ 无版本字段 |
| **Opencode** | `1.2.27` | ✅ `session.version` (如 `"1.2.27"`) |

### 版本存储位置

**Opencode (唯一在会话数据中存储版本的工具)**:

- **SQLite (1.2+)**: `session.version` 字段
  ```sql
  SELECT id, version, time_created FROM session;
  -- 示例：ses_30a6feca7ffekucAfkRQ3Jqku7 | 1.2.27 | 1773646386008
  ```

- **文件系统 (1.1.x)**: session JSON 文件的 `version` 字段
  ```json
  {
    "id": "ses_xxx",
    "version": "1.1.56",
...
**其他工具**:
- Codex、Claude、Gemini 的会话数据中**不包含版本号**
- 版本信息只能通过 CLI 工具的 `--version` 命令获取
- 会话格式变更需要通过数据结构推断（如新增字段）
...
**重大变更**:
- **1.2.0**: 从文件系统迁移到 SQLite 数据库
- **1.1.x**: 使用 `~/.local/share/opencode/storage/` 目录
