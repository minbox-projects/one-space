...
import { maskSecret, type GatewayKey } from "@/lib/aiGateway";
...
export function LocalKeyList({
  ...
  return (
    <section className="space-y-3.5" data-testid="ai-gateway-keys">
      {/* 头部说明与添加栏 */}
      ...
      {keys.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed bg-card/50 px-6 py-10 text-center">
          ...
          <h4 className="mt-2.5 text-xs font-medium text-foreground">
            {t("aiGatewayNoKeys", "No local keys yet.")}
          </h4>
          <p className="mt-1 max-w-sm text-xs text-muted-foreground">
            {t(
              "aiGatewayNoKeysGuide",
        ...
        <div className="space-y-2">
          {keys.map((key) => {
            ...
            return (
              <div
                key={key.id}
                data-testid={`ai-gateway-key-${key.id}`}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {isDefault ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[11px] font-medium leading-4 text-primary">
                        ...
                        {t("aiGatewayDefaultKey", "Default key")}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <code
                      ...
                      data-testid={`ai-gateway-key-value-${key.id}`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => onCopy(key)}
                      aria-label={t("aiGatewayCopyKeyAria", {
                        label: key.label,
                        defaultValue: `Copy key ${key.label}`,
                      })}
                      title={t("aiGatewayCopyKey", "Copy key")}
                ...
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-muted-foreground">
                      {key.enabled
                        ? t("aiGatewayEnabled", "Enabled")
                        : t("aiGatewayDisabled", "Disabled")}
                    </span>
                    <Switch
                      aria-label={t("aiGatewayToggleKeyAria", {
                        label: key.label,
                  ...
                  {!isDefault && key.enabled ? (
                    ...
                    >
                      {t("aiGatewaySetDefault", "Set as default")}
                    </button>
                  ) : null}
                  ...
                  <button
                    ...
                    aria-label={t("aiGatewayDeleteKeyAria", {
                      label: key.label,
      ...
      <div className="rounded-lg border bg-muted/20 p-2.5 text-xs text-muted-foreground">
        <span className="font-medium text-foreground">
          {t("aiGatewayUsageTipTitle", "How to use:")}
        </span>{" "}
        {t(
          "aiGatewayUsageTipDesc",
