...
import { MappingPriceEditor } from "@/components/AiGateway/MappingPriceEditor";
import type { GatewayPriceDraft } from "@/lib/aiGateway";
...
describe("MappingPriceEditor 映射行价格编辑器", () => {
  ...
  it("mapping_price_editor_edits_tiers_and_reports_patches", async () => {
    ...
    await user.type(screen.getByTestId("ai-gateway-price-0-cache-read"), "0.2");
    await user.type(screen.getByTestId("ai-gateway-price-0-cache-write"), "0.3");
    ...
    expect(valueOf("ai-gateway-price-0-cache-read")).toBe("0.2");
    expect(valueOf("ai-gateway-price-0-cache-write")).toBe("0.3");
  ...
  it("mapping_price_editor_toggles_off_peak_and_adds_default_window", async () => {
    ...
    expect(
      screen.queryByTestId("ai-gateway-off-peak-0-start-0"),
    ...
    expect(screen.getByTestId("ai-gateway-off-peak-0-start-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-off-peak-0-end-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-off-peak-0-input-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-off-peak-0-cache-read-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-off-peak-0-cache-write-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-off-peak-0-output-0")).toBeInTheDocument();

    await user.click(screen.getByTestId("ai-gateway-off-peak-0-add"));
    expect(lastOffPeaks(onChange)).toHaveLength(2);

    await user.click(screen.getByTestId("ai-gateway-off-peak-0-remove-1"));
  ...
  it("mapping_price_editor_weekday_chips_and_every_day", async () => {
    ...
    const dayChip = (day: number) =>
      screen.getByTestId(`ai-gateway-off-peak-0-day-0-${day}`);
    ...
    await user.click(screen.getByTestId("ai-gateway-off-peak-0-everyday-0"));
  ...
  it("mapping_price_editor_echoes_old_data", () => {
    ...
    expect(valueOf("ai-gateway-price-0-input")).toBe("0");
    expect(valueOf("ai-gateway-price-0-cache-read")).toBe("0");
    expect(valueOf("ai-gateway-price-0-cache-write")).toBe("0");
    expect(valueOf("ai-gateway-price-0-output")).toBe("0");
    expect(valueOf("ai-gateway-off-peak-0-start-0")).toBe("23:00");
    expect(valueOf("ai-gateway-off-peak-0-end-0")).toBe("07:00");
    expect(valueOf("ai-gateway-off-peak-0-input-0")).toBe("0.5");
    expect(valueOf("ai-gateway-off-peak-0-output-0")).toBe("2");
    expect(
      screen.getByTestId("ai-gateway-off-peak-0-day-0-1"),
    ).toHaveAttribute("aria-pressed", "true");
    expect(
      screen.getByTestId("ai-gateway-off-peak-0-day-0-2"),
    ).toHaveAttribute("aria-pressed", "true");
  });
});
