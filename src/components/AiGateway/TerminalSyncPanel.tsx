...
import {
  AI_GATEWAY_SUPPORTED_TERMINAL_TOOLS,
...
export function TerminalSyncPanel({
  ...
  const supportedTargets = targets.filter((target) =>
    AI_GATEWAY_SUPPORTED_TERMINAL_TOOLS.includes(
      target.tool as (typeof AI_GATEWAY_SUPPORTED_TERMINAL_TOOLS)[number],
    ),
  );
    ...
    <div className="space-y-3.5">
        ...
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">
              {t("aiGatewayTerminalSync", "AI terminal integration")}
            </h3>
          ...
          <p className="text-xs text-muted-foreground">
            {t(
              ...
              "Each tool shows Add or Sync on its right based on its state. Syncing writes the gateway endpoint, default key, and model mappings as an independent provider, and it is not activated automatically. If its gateway provider was deleted, syncing creates a new AI Gateway provider.",
            )}
          </p>
        </div>
      </div>
      ...
      {defaultKeyMissing ? (
        <div
          ...
          data-testid="ai-gateway-default-key-required"
          role="alert"
        >
          <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
          <div>
            <div className="font-semibold text-xs">
              {t("aiGatewayDefaultKeyRequiredTitle", "Local default key required")}
            </div>
            <div className="mt-0.5 text-[11px]">
              {t(
                "aiGatewayDefaultKeyRequired",
      ...
      {gatewayRunning === false ? (
        <div
          ...
          data-testid="ai-gateway-terminal-stopped-warning"
          role="alert"
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {onStartGateway ? (
              <button
                ...
                data-testid="ai-gateway-start-from-terminals"
              ...
              >
                ...
                <span>{t("aiGatewayStartServiceNow", "立即启动服务")}</span>
              </button>
            ) : null}
          </div>
        </div>
      ...
      {/* 目标列表 */}
      <section data-testid="ai-gateway-terminals">
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed bg-card/50 px-6 py-10 text-center">
          ...
          <h4 className="mt-2.5 text-xs font-medium text-foreground">
            {t("aiGatewayNoTerminalTargets", "No OpenCode or Codex terminal targets found.")}
          </h4>
          <p className="mt-1 max-w-sm text-xs text-muted-foreground">
            {t(
              "aiGatewayNoTerminalTargetsGuide",
        ...
        <div className="space-y-2">
          {supportedTargets.map((target) => {
            ...
            return (
              <div
                key={target.tool}
                data-testid={`ai-gateway-target-${target.tool}`}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                    <span className="truncate font-mono">
                      {target.base_url ?? t("aiGatewayNoValue", "not set")}
                    </span>
                    <span className="text-muted-foreground/30">•</span>
                    <span
                      data-testid={`ai-gateway-target-synced-${target.tool}`}
                      className="inline-flex items-center gap-1 text-[11px]"
                    >
                      <span className="shrink-0">
                        {t("aiGatewayTerminalLastSync", "Last sync")}:
                      </span>
                      <span className="font-medium text-foreground/80">
                        {target.synced_at
                          ? formatGatewayTimestamp(target.synced_at)
                          : t("aiGatewayTerminalNotSynced", "Not synced yet")}
                      </span>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    ...
                    data-testid={`ai-gateway-target-status-${target.tool}`}
                  >
                    {pending ? (
                      <>
                        <Clock className="h-3 w-3" />
                        {t("aiGatewayPendingSync", "Pending sync")}
                      </>
                    ) : (
                      <>
                        <CheckCircle2 className="h-3 w-3" />
                        {t("aiGatewaySynced", "Synced")}
                      </>
                    )}
                  </span>
                  {hasBeenAdded ? (
                    <button
                      ...
                      data-testid={`ai-gateway-sync-${target.tool}`}
                    ...
                    >
                      <Wand2
                        className={`h-3.5 w-3.5 ${isSyncing ? "animate-spin" : ""}`}
                      />
                      {t("aiGatewayConfigureSelected", "Add provider")}
                    </button>
                  )}
                </div>
              </div>
      ...
      {/* 终端连接常见问题排查指引 */}
      <div className="rounded-xl border bg-muted/20 p-3 text-xs" data-testid="ai-gateway-terminal-faq">
        <button
          type="button"
          onClick={() => setShowFaq((prev) => !prev)}
          className="flex w-full items-center justify-between font-semibold text-foreground hover:text-primary transition"
          aria-expanded={showFaq}
          data-testid="ai-gateway-terminal-faq-toggle"
        >
          <div className="flex items-center gap-2">
            <HelpCircle className="h-3.5 w-3.5 text-muted-foreground" />
            <span>{t("aiGatewayTerminalFaqTitle", "终端调用常见报错排查")}</span>
          </div>
          <ChevronDown
        ...
        {showFaq ? (
          <div
            ...
            data-testid="ai-gateway-terminal-faq-content"
          >
            <div className="space-y-0.5">
              <div className="font-medium text-foreground">
                Q: {t("aiGatewayFaqQ1", "终端提示 Cannot connect to API: Unable to connect. Is the computer able to access the url?")}
              </div>
              <p>
                {t(
                  "aiGatewayFaqA1",
            ...
            <div className="space-y-0.5">
              <div className="font-medium text-foreground">
                Q: {t("aiGatewayFaqQ2", "终端提示 all providers unavailable: 429 或额度用尽？")}
              </div>
              <p>
                {t(
                  "aiGatewayFaqA2",
