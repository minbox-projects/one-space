...
export function UpstreamProviderDetail({
  ...
  return (
    <section className="rounded-[24px] border bg-card p-5" data-testid="ai-gateway-provider-detail">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayName", "Name")}</span>
          <input
            type="text"
            value={name}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayBaseUrl", "API base URL")}</span>
          <input
            type="text"
            value={baseUrl}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayApiKey", "API key")}</span>
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayDefaultModel", "Default model")}</span>
          <input
            type="text"
            value={defaultModel}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayProtocol", "API protocol")}</span>
          <select
            value={protocol}
            onChange={(event) =>
      ...
      <div className="mt-5 space-y-2">
        <div className="flex items-center justify-between gap-3">
          <span className={labelClass}>{t("aiGatewayModelMappings", "Model mappings")}</span>
          <button
            type="button"
            onClick={() =>
      ...
      <div className="mt-5 grid gap-3 rounded-2xl border bg-muted/10 p-4 md:grid-cols-3">
        <label className="space-y-1">
          <span className={labelClass}>{t("aiGatewayPreviewModel", "Preview model")}</span>
          <input
            type="text"
            value={previewModel}
            onChange={(event) => setPreviewModel(event.target.value)}
            aria-label={t("aiGatewayPreviewModel", "Preview model")}
            className={`${inputClass} font-mono`}
          />
        </label>
        <div className="space-y-1">
          <span className={labelClass}>{t("aiGatewayPreviewResult", "Resolved upstream model")}</span>
          <div
            className={`rounded-md border px-3 py-2 font-mono text-sm ${
              preview ? "text-foreground" : "text-muted-foreground"
            }`}
            data-testid="ai-gateway-model-preview"
          >
            {preview?.upstreamModel ?? t("aiGatewayPreviewUnavailable", "Not resolvable")}
          </div>
        </div>
        <div className="space-y-1">
          <span className={labelClass}>{t("aiGatewayPreviewEndpoint", "Target endpoint")}</span>
          <div
            className={`rounded-md border px-3 py-2 font-mono text-sm ${
              preview ? "text-foreground" : "text-muted-foreground"
            }`}
            data-testid="ai-gateway-endpoint-preview"
          >
            {preview
              ? endpointPath(preview.endpoint)
              : t("aiGatewayPreviewUnavailable", "Not resolvable")}
          </div>
        </div>
      </div>
