...
import { UsageStatsPanel } from "@/components/AiGateway/UsageStatsPanel";
import type { UsageStats } from "@/lib/aiGateway";
...
describe("UsageStatsPanel", () => {
  ...
  it("默认今日并在切范围时按 range 重新取数，卡片展示 Tokens/请求数/花费", async () => {
    ...
    invokeMock.mockImplementation(async (command: string, args?: any) => {
      if (command !== "ai_gateway_usage_stats") {
    ...
    const tokensCard = await screen.findByTestId("ai-gateway-usage-card-tokens");
    expect(tokensCard).toHaveTextContent(formatCount(1000));
    expect(screen.getByTestId("ai-gateway-usage-card-requests")).toHaveTextContent(
      formatCount(4),
    );
    expect(screen.getByTestId("ai-gateway-usage-card-cost")).toHaveTextContent(
      "0.1234",
    );

    const rangeTrigger = screen.getByTestId("ai-gateway-usage-range-trigger");
    ...
    expect(
      await screen.findByTestId("ai-gateway-usage-card-requests"),
  ...
  it("范围下拉提供昨天并按 range=yesterday 取数渲染返回指标", async () => {
    ...
    expect(screen.getByTestId("ai-gateway-usage-card-tokens")).toHaveTextContent(
      formatCount(777),
    );
    expect(screen.getByTestId("ai-gateway-usage-card-cost")).toHaveTextContent(
      "0.5",
  ...
  it("刷新按钮在请求期间展示刷新中状态并重新取数", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("ai-gateway-usage-card-requests"),
  ...
  it("未定价模型行显示破折号且不计入合计，同时提示未定价请求数", async () => {
    ...
    const modelTable = await screen.findByTestId("ai-gateway-usage-models");
    ...
    const hint = screen.getByTestId("ai-gateway-usage-unpriced-hint");
  ...
  it("未配置价格提示中展示服务商名称、模型 ID 以及上游模型和请求数", async () => {
    ...
    const hint = await screen.findByTestId("ai-gateway-usage-unpriced-hint");
    expect(hint).toHaveTextContent("5 requests have no configured price");
    expect(hint).toHaveTextContent("Unconfigured models:");

    const items = within(hint).getAllByTestId("ai-gateway-usage-unpriced-item");
  ...
  it("单日按 UTC+8 小时展示有数据的小时且不超过 24 行", async () => {
    ...
    const bucketTable = await screen.findByTestId("ai-gateway-usage-buckets");
    const rows = within(bucketTable).getAllByTestId("ai-gateway-usage-bucket-row");
  ...
  it("usage_stats_panel_has_no_model_prices_entry", async () => {
    ...
    expect(
      screen.queryByTestId("ai-gateway-model-price-dialog"),
    ).toBeNull();
    expect(invokeMock).not.toHaveBeenCalledWith("ai_gateway_model_prices_get");
  });
  ...
  it("每个模型的提供商明细紧跟其模型行之后渲染", async () => {
    ...
    const rows = within(modelTable).getAllByTestId(
      /ai-gateway-usage-(model|provider)-row/,
    );
  ...
  it("Tokens 卡片、时间分布与用量分析行正确应用单位转换格式并展示完整数值 title", async () => {
    ...
    // 时间分布表格
    const bucketTable = screen.getByTestId("ai-gateway-usage-buckets");
    const bucketRow = within(bucketTable).getByTestId("ai-gateway-usage-bucket-row");
    ...
    // 模型分析表格
    const modelTable = screen.getByTestId("ai-gateway-usage-models");
    const modelRow = within(modelTable).getByTestId("ai-gateway-usage-model-row");
  ...
  it("展示细分指标卡片（Token 缓存命中率、Input、Output、Cache Read、Cache Write）", async () => {
    ...
    const cacheHitCard = await screen.findByTestId("ai-gateway-usage-card-cache-hit");
    ...
    const inputCard = screen.getByTestId("ai-gateway-usage-card-input");
    expect(inputCard).toHaveTextContent("300");

    const cacheReadCard = screen.getByTestId("ai-gateway-usage-card-cache-read");
    expect(cacheReadCard).toHaveTextContent("100");

    const cacheWriteCard = screen.getByTestId("ai-gateway-usage-card-cache-write");
    expect(cacheWriteCard).toHaveTextContent("50");

    const outputCard = screen.getByTestId("ai-gateway-usage-card-output");
  ...
  it("渲染时间趋势柱状图并在切换维度时更新柱条", async () => {
    ...
    const trendCard = await screen.findByTestId("ai-gateway-usage-trend-card");
    expect(trendCard).toBeInTheDocument();

    // 峰值徽标应该指示最高的一天 2026-09-20
    const peakBadge = screen.getByTestId("ai-gateway-usage-peak-badge");
    expect(peakBadge).toHaveTextContent("2026-09-20");

    const bars = screen.getAllByTestId("ai-gateway-usage-trend-bar");
    expect(bars).toHaveLength(2);

    // 切换到 Requests 维度
    const requestsBtn = screen.getByTestId("ai-gateway-trend-view-requests");
    await user.click(requestsBtn);

    // 切换到 Cost 维度
    const costBtn = screen.getByTestId("ai-gateway-trend-view-cost");
    await user.click(costBtn);
    expect(bars[1]).toHaveClass("bg-emerald-600/70");

    // 切回 Tokens 维度
    const tokensBtn = screen.getByTestId("ai-gateway-trend-view-tokens");
  ...
  it("支持展开和收起时间分布详细明细表格", async () => {
    ...
    // 初始状态下表格可见
    expect(await screen.findByTestId("ai-gateway-usage-buckets")).toBeInTheDocument();

    const toggleBtn = screen.getByTestId("ai-gateway-toggle-bucket-table");
    expect(toggleBtn).toHaveTextContent("Hide Details");

    // 点击收起
    await user.click(toggleBtn);
    expect(screen.queryByTestId("ai-gateway-usage-buckets")).not.toBeInTheDocument();
    expect(toggleBtn).toHaveTextContent("Show Details");

    // 点击再次展开
    await user.click(toggleBtn);
    expect(screen.getByTestId("ai-gateway-usage-buckets")).toBeInTheDocument();
  });
  ...
  it("模型与服务商分析行中展示占比进度条", async () => {
    ...
    const shareBars = await screen.findAllByTestId("ai-gateway-usage-share-bar");
  ...
  it("用量分析列表使用后端缓存命中率与覆盖计数，不再前端换算", async () => {
    ...
    const modelRow = within(modelTable).getByTestId("ai-gateway-usage-model-row");
    const modelCacheHitCell = screen.getByTestId("ai-gateway-usage-model-cache-hit");
    ...
    const providerRow = within(modelTable).getByTestId("ai-gateway-usage-provider-row");
    const providerCacheHitCell = screen.getByTestId("ai-gateway-usage-provider-cache-hit");
  ...
  it("无有效分母时命中率显示破折号且覆盖为 0 / 成功请求数，绝不显示 0%", async () => {
    ...
    const modelCacheHitCell = within(modelRow).getByTestId(
      "ai-gateway-usage-model-cache-hit",
    );
    ...
    const providerRow = within(modelTable).getByTestId(
      "ai-gateway-usage-provider-row",
    );
    const providerCacheHitCell = within(providerRow).getByTestId(
      "ai-gateway-usage-provider-cache-hit",
    );
  ...
  it("后端合法的 0.0 显示为 0% 且与无数据的破折号可区分", async () => {
    ...
    const modelTable = await screen.findByTestId("ai-gateway-usage-models");
    const modelCacheHitCell = within(modelTable).getByTestId("ai-gateway-usage-model-cache-hit");
  ...
  it("脏数据中空服务商行不渲染，含数据的真实服务商行仍保留", async () => {
    ...
    expect(
      within(modelTable).getAllByTestId("ai-gateway-usage-model-row"),
    ).toHaveLength(2);
