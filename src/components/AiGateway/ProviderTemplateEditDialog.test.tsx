...
describe("ProviderTemplateEditDialog", () => {
  ...
  it("expands mapping row to edit pricing and reasoning efforts and saves them", async () => {
    ...
    // 默认未展开，价格与推理档位不显示
    expect(screen.queryByTestId("ai-gateway-mapping-price-0")).not.toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-mapping-efforts-0")).not.toBeInTheDocument();

    // 点击展开第 1 个模型
    fireEvent.click(screen.getByTestId("template-edit-model-expand-0"));

    expect(screen.getByTestId("ai-gateway-mapping-price-0")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-mapping-efforts-0")).toBeInTheDocument();
    ...
    const inputPrice = screen.getByTestId("ai-gateway-price-0-input");
    const outputPrice = screen.getByTestId("ai-gateway-price-0-output");
    ...
    fireEvent.click(screen.getByTestId("ai-gateway-mapping-effort-add-0"));

    expect(screen.getByTestId("ai-gateway-mapping-effort-0-medium")).toBeInTheDocument();
  ...
  it("creates a new template with model mappings, prices and protocols", async () => {
    ...
    fireEvent.change(screen.getByTestId("ai-gateway-price-0-output"), {
      target: { value: "15" },
    });
