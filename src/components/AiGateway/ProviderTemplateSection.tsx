...
function ProviderTemplateCard({
  ...
  const handleCopyModel = async (modelName: string) => {
    try {
      ...
      pushToast({
        title: t("aiGatewayTemplateModelCopied", "Model identifier copied"),
  ...
  return (
    <div
      data-testid={`ai-gateway-template-${template.id}`}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 border-b border-border/40 bg-muted/20 px-4.5 py-3.5 sm:px-5">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {onEditTemplate && (
            <button
              type="button"
              data-testid={`ai-gateway-template-edit-${template.id}`}
              onClick={() => onEditTemplate(template)}
              disabled={busy}
              aria-label={t("aiGatewayEditTemplate", "Edit template")}
              title={t("aiGatewayEditTemplate", "Edit template")}
          ...
          {template.models_url?.trim() ? (
            <button
              type="button"
              data-testid={`ai-gateway-template-sync-${template.id}`}
              onClick={() => onSync(template.id)}
              disabled={syncing}
              aria-label={
                syncing
                  ? t("aiGatewayTemplateSyncing", "Syncing models...")
                  : t("aiGatewayTemplateSync", "Sync models")
              }
              title={t("aiGatewayTemplateSync", "Sync models")}
      ...
      <div className="flex-1 px-4.5 py-4 sm:px-5 space-y-3.5">
        {template.description ? (
          ...
          <p className="text-xs italic text-muted-foreground/60">
            {t("aiGatewayNoDescription", "No description provided")}
          </p>
        )}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-0.5 text-xs">
          ...
          {template.source && (
            ...
            >
              <span className="text-muted-foreground/80 font-medium shrink-0">
                {t("aiGatewayTemplateSource", "Source")}:
              </span>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-md border border-border/60 bg-muted/40 px-2.5 py-1 text-[11px] text-muted-foreground">
            <span className="shrink-0">{t("aiGatewayTemplateLastSync", "Last sync")}:</span>
            <span
              data-testid={`ai-gateway-template-synced-${template.id}`}
        ...
        {failureReason ? (
          <div
            data-testid={`ai-gateway-template-auto-refresh-failure-${template.id}`}
            className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-400"
          >
            {t("aiGatewayTemplateAutoRefreshFailed", { reason: failureReason })}
          </div>
        ) : null}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-border/50 bg-muted/20 px-4.5 py-2.5 sm:px-5">
        <button
          type="button"
          data-testid={`ai-gateway-template-expand-${template.id}`}
        ...
        >
          ...
          <span>
            {expanded
              ? t("aiGatewayTemplateCollapse", "Hide template models")
              : t("aiGatewayTemplateExpand", "Show template models")}
          </span>
        ...
        <button
          type="button"
          data-testid={`ai-gateway-template-add-${template.id}`}
        ...
        >
          ...
          <span>{t("aiGatewayTemplateAddProvider", "Add as upstream provider")}</span>
        </button>
      </div>
      ...
      {expanded ? (
        <div className="border-t border-border/70 bg-muted/15 p-4 sm:p-5 space-y-3">
          {template.models.length === 0 ? (
            <p
              data-testid={`ai-gateway-template-no-models-${template.id}`}
              className="rounded-xl border border-dashed bg-card/50 p-6 text-center text-xs text-muted-foreground"
            >
              {t("aiGatewayTemplateNoModels", "No models in this template")}
            </p>
          ) : (
            <>
              {/* 过滤工具栏：搜索框与协议筛选 */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input
                    ...
                    placeholder={t(
                      "aiGatewayTemplateFilterPlaceholder",
                ...
                >
                  ...
                  >
                    {t("aiGatewayTemplateProtocolAll", "All")} ({template.models.length})
                  </button>
                  {chatCount > 0 && (
                    <button
                      type="button"
              ...
              <div className="max-h-80 overflow-y-auto rounded-xl border border-border/70 bg-card divide-y divide-border/40 shadow-xs">
                {filteredModels.length === 0 ? (
                  ...
                  >
                    <p>
                      {t(
                        "aiGatewayTemplateNoMatchingModels",
                    ...
                    >
                      {t("aiGatewayTemplateClearFilter", "Clear filter")}
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    {filteredModels.map((model) => {
                      ...
                      return (
                        <div
                          ...
                          data-testid={`ai-gateway-template-model-${template.id}-${model.upstream_model}`}
                        ...
                        >
                          ...
                          <div className="flex min-w-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              title={t(
                                "aiGatewayTemplateCopyModelName",
...
export function ProviderTemplateSection({
  ...
  return (
    <section className="space-y-4" data-testid="ai-gateway-provider-templates">
      ...
      {templates.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed bg-card/40 px-6 py-12 text-center">
          ...
          <h4 className="mt-3 text-xs font-medium text-foreground">
            {t("aiGatewayNoTemplates", "No provider templates available.")}
          </h4>
        </div>
      ) : (
