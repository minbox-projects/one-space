...
describe("ProviderTemplateSection 服务商模板区域", () => {
  ...
  it("rendersTemplateCardsWithMetadataAndNoSnapshotBadges", () => {
    ...
    const snapshotCard = screen.getByTestId("ai-gateway-template-t1");
    ...
    expect(snapshotCard).toHaveTextContent(i18n.t("aiGatewayTemplateNotSynced"));
    expect(snapshotCard).not.toHaveTextContent(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}/);

    // 离线快照与快照版本徽标必须彻底移除
    expect(screen.queryByTestId("ai-gateway-template-snapshot-t1")).not.toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-snapshot-version-t1")).not.toBeInTheDocument();
    expect(screen.queryByText("Offline snapshot")).not.toBeInTheDocument();

    const syncedCard = screen.getByTestId("ai-gateway-template-t2");
    ...
    expect(screen.getByTestId("ai-gateway-provider-templates")).toBeInTheDocument();
  });
  ...
  it("syncActionRendersOnlyWhenModelsUrlIsConfigured", () => {
    ...
    expect(screen.queryByTestId("ai-gateway-template-sync-t-blank")).not.toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-sync-t-space")).not.toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-sync-t-null")).not.toBeInTheDocument();

    const syncButton = screen.getByTestId("ai-gateway-template-sync-t-defined");
  ...
  it("syncActionUsesLocalizedLabel", async () => {
    ...
    expect(screen.getByTestId("ai-gateway-template-sync-t1")).toHaveTextContent(
      "同步模型列表",
    );
  });
  ...
  it("expandedTemplateListsModelsWithoutPricesOffPeakOrReasoningEfforts", () => {
    ...
    const expand = screen.getByTestId("ai-gateway-template-expand-t1");
    expect(expand).toHaveAttribute("aria-expanded", "false");
    fireEvent.click(expand);
    expect(expand).toHaveAttribute("aria-expanded", "true");

    const row = screen.getByTestId("ai-gateway-template-model-t1-deepseek-chat");
  ...
  it("disabledModelRowIsDeemphasizedAndMarked", () => {
    ...
    renderSection({ templates: [view] });
    fireEvent.click(screen.getByTestId("ai-gateway-template-expand-t1"));

    const disabledRow = screen.getByTestId(
      "ai-gateway-template-model-t1-disabled-model",
    );
    expect(
    ...
    const enabledRow = screen.getByTestId(
      "ai-gateway-template-model-t1-enabled-model",
    );
    expect(
  ...
  it("syncButtonIsolatesBusyStatePerTemplate", () => {
    ...
    const buttonA = screen.getByTestId("ai-gateway-template-sync-t1");
    const buttonB = screen.getByTestId("ai-gateway-template-sync-t2");
    expect(buttonA).toBeDisabled();
    expect(buttonA).toHaveTextContent(i18n.t("aiGatewayTemplateSyncing"));
    expect(buttonB).toBeEnabled();
    // 同步中的模板仍可重开添加弹窗，busy 只作用于当前模板的同步按钮
    expect(screen.getByTestId("ai-gateway-template-add-t1")).toBeEnabled();
  ...
  it("emptyTemplatesShowsEmptyState", () => {
    ...
    const root = screen.getByTestId("ai-gateway-provider-templates");
    expect(root).toBeInTheDocument();
    expect((root.textContent ?? "").trim().length).toBeGreaterThan(0);
    expect(screen.queryByTestId("ai-gateway-template-t1")).not.toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-expand-t1")).not.toBeInTheDocument();
  });

  it("createDialogRejectsEmptyApiKeyWithoutClosing", async () => {
    const { onCreateProvider } = renderSection();

    fireEvent.click(screen.getByTestId("ai-gateway-template-add-t1"));
    const keyInput = await screen.findByTestId("ai-gateway-template-api-key");
    fireEvent.change(keyInput, { target: { value: "   " } });

    fireEvent.click(screen.getByTestId("ai-gateway-template-create-submit"));

    await screen.findByTestId("ai-gateway-template-api-key-error");
    expect(onCreateProvider).not.toHaveBeenCalled();
    expect(screen.getByTestId("ai-gateway-template-api-key")).toBeInTheDocument();
  });
  ...
  it("createDialogSubmitsPrefilledRequestAndClosesOnSuccess", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByTestId("ai-gateway-template-api-key")).not.toBeInTheDocument(),
    );
  });
  ...
  it("zeroModelTemplateShowsDedicatedNoModelsHint", () => {
    ...
    fireEvent.click(screen.getByTestId("ai-gateway-template-expand-t-empty"));

    const hint = screen.getByTestId("ai-gateway-template-no-models-t-empty");
  ...
  it("filtersModelsBySearchTermAndClearsFilter", () => {
    ...
    expect(screen.getByTestId("ai-gateway-template-model-t1-deepseek-chat")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-template-model-t1-claude-3-opus")).toBeInTheDocument();

    const searchInput = screen.getByTestId("template-model-search-t1");
    fireEvent.change(searchInput, { target: { value: "deepseek" } });

    expect(screen.getByTestId("ai-gateway-template-model-t1-deepseek-chat")).toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-model-t1-claude-3-opus")).not.toBeInTheDocument();

    fireEvent.change(searchInput, { target: { value: "not-exist" } });
    expect(screen.queryByTestId("ai-gateway-template-model-t1-deepseek-chat")).not.toBeInTheDocument();
    expect(screen.getByTestId("template-no-matching-models-t1")).toBeInTheDocument();

    // 点击清除筛选
    fireEvent.click(screen.getByText(i18n.t("aiGatewayTemplateClearFilter")));
    expect(screen.getByTestId("ai-gateway-template-model-t1-deepseek-chat")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-template-model-t1-claude-3-opus")).toBeInTheDocument();
  });
  ...
  it("filtersModelsByProtocolPills", () => {
    ...
    expect(screen.getByTestId("ai-gateway-template-model-t1-chat-model")).toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-template-model-t1-resp-model")).toBeInTheDocument();

    // 切换到 Responses
    fireEvent.click(screen.getByRole("button", { name: /Responses/ }));
    expect(screen.queryByTestId("ai-gateway-template-model-t1-chat-model")).not.toBeInTheDocument();
    expect(screen.getByTestId("ai-gateway-template-model-t1-resp-model")).toBeInTheDocument();

    // 切换到 Chat
    fireEvent.click(screen.getByRole("button", { name: /^Chat/ }));
    expect(screen.getByTestId("ai-gateway-template-model-t1-chat-model")).toBeInTheDocument();
    expect(screen.queryByTestId("ai-gateway-template-model-t1-resp-model")).not.toBeInTheDocument();
  ...
  it("togglesAllTemplatesExpandedState", () => {
    ...
    const expand1 = screen.getByTestId("ai-gateway-template-expand-t1");
    const expand2 = screen.getByTestId("ai-gateway-template-expand-t2");
  ...
  it("renders corresponding template icon in card header", () => {
    ...
    const card = screen.getByTestId("ai-gateway-template-tpl-cmd");
  ...
  it("rendersAutoRefreshFailureInlineOnMatchingCardAndKeepsSyncedAt", () => {
    ...
    expect(
      screen.queryByTestId("ai-gateway-template-auto-refresh-failure-t2"),
    ...
    expect(screen.getByTestId("ai-gateway-template-synced-t2")).toHaveTextContent(
      syncedText,
    );
  ...
  it("localizesAutoRefreshFailureWithInterpolatedReason", async () => {
    ...
    expect(failure.textContent ?? "").not.toContain(
      "aiGatewayTemplateAutoRefreshFailed",
    );
