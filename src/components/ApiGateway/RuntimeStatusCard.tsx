  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 rounded-lg border bg-muted/20 px-3 py-1.5 text-xs">
            ...
            <span className="text-muted-foreground/70 text-[11px] font-medium">
              {t("apiGatewayDefaultKey", "Default API key")}:
            </span>
            {effectiveDefaultKey ? (
              <div className="flex items-center gap-1">
                <code
                  ...
                  data-testid="api-gateway-default-key-preview"
                >
                ...
                <button
                  ...
                  aria-label={t("apiGatewayCopyDefaultKey", "Copy default API key")}
                  title={
                    defaultKeyCopied
                      ? t("apiGatewayCopied", "Copied")
                      : t("apiGatewayCopyDefaultKey", "Copy default API key")
                  }
                  data-testid="api-gateway-copy-default-key"
                  className="inline-flex h-5 w-5 items-center justify-center rounded text-muted-foreground transition hover:bg-muted hover:text-foreground"
                >
                  {defaultKeyCopied ? (
                    <Check className="h-3 w-3 text-emerald-600 dark:text-emerald-400" />
              ...
              >
                {t("apiGatewayNoDefaultKey", "No key configured")}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {onRefresh && (
            <button
              ...
              title={t("apiGatewayRefreshData", "Refresh data")}
              aria-label={t("apiGatewayRefreshData", "Refresh data")}
              data-testid="api-gateway-refresh-btn"
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
        ...
        {/* 指标卡 3：今日请求量（高频业务指标） */}
        <div
          data-testid="api-gateway-metric-requests"
        ...
        >
          <div className="flex items-center justify-between text-muted-foreground">
            <span className="text-[11px] font-medium">
              {t("apiGatewayTodayRequests", "Today's requests")}
            </span>
            ...
            {todayRequests > 0 ? (
              todayFailedRequests > 0 ? (
                <span
                  ...
                  data-testid="api-gateway-today-failed-requests"
                >
                  <AlertTriangle className="h-3 w-3 shrink-0" />
                  <span>
                    {t("apiGatewayTodayFailureCount", {
                ...
                <span className="inline-flex items-center gap-1 font-medium text-emerald-600 dark:text-emerald-400">
                  ...
                  <span>{t("apiGatewayTodaySuccessRate", "100% success")}</span>
                </span>
              )
            ) : (
              <span className="text-muted-foreground">{t("apiGatewayUsageEmpty", "No usage records")}</span>
            )}
          </div>
        </div>
        ...
        <div
          data-testid="api-gateway-metric-tokens"
          onClick={() => onSelectTab?.("usage")}
          role={onSelectTab ? "button" : undefined}
          tabIndex={onSelectTab ? 0 : undefined}
          onKeyDown={(e) => {
        ...
        >
          <div className="flex items-center justify-between text-muted-foreground">
            <span className="text-[11px] font-medium">
              {t("apiGatewayTodayTokens", "Today's tokens")}
            </span>
