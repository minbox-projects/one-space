...
export function UpstreamProviderDetail({
  ...
  const [protocol, setProtocol] = useState<GatewayUpstreamProtocol>(
    provider.protocol ?? "chat_completions",
  );
  const [mappings, setMappings] = useState<GatewayModelMapping[]>(provider.mappings);
  ...
  return (
    <section className="rounded-[24px] border bg-card p-5" data-testid="api-gateway-provider-detail">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayName", "Name")}</span>
          <input
            type="text"
            value={name}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayBaseUrl", "API base URL")}</span>
          <input
            type="text"
            value={baseUrl}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayApiKey", "API key")}</span>
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayDefaultModel", "Default model")}</span>
          <input
            type="text"
            value={defaultModel}
        ...
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayProtocol", "API protocol")}</span>
          <select
            value={protocol}
            onChange={(event) =>
      ...
      <div className="mt-5 space-y-2">
        <div className="flex items-center justify-between gap-3">
          <span className={labelClass}>{t("apiGatewayModelMappings", "Model mappings")}</span>
          <button
            type="button"
            onClick={() =>
      ...
      <div className="mt-5 grid gap-3 rounded-2xl border bg-muted/10 p-4 md:grid-cols-3">
        <label className="space-y-1">
          <span className={labelClass}>{t("apiGatewayPreviewModel", "Preview model")}</span>
          <input
            type="text"
            value={previewModel}
            onChange={(event) => setPreviewModel(event.target.value)}
            aria-label={t("apiGatewayPreviewModel", "Preview model")}
            className={`${inputClass} font-mono`}
          />
        </label>
        <div className="space-y-1">
          <span className={labelClass}>{t("apiGatewayPreviewResult", "Resolved upstream model")}</span>
          <div
            className={`rounded-md border px-3 py-2 font-mono text-sm ${
              preview ? "text-foreground" : "text-muted-foreground"
            }`}
            data-testid="api-gateway-model-preview"
          >
            {preview?.upstreamModel ?? t("apiGatewayPreviewUnavailable", "Not resolvable")}
          </div>
        </div>
        <div className="space-y-1">
          <span className={labelClass}>{t("apiGatewayPreviewEndpoint", "Target endpoint")}</span>
          <div
            className={`rounded-md border px-3 py-2 font-mono text-sm ${
              preview ? "text-foreground" : "text-muted-foreground"
            }`}
            data-testid="api-gateway-endpoint-preview"
          >
            {preview
              ? endpointPath(preview.endpoint)
              : t("apiGatewayPreviewUnavailable", "Not resolvable")}
          </div>
        </div>
      </div>
