...
function normalizeSourceNameMap(config: StorageConfigLite | null | undefined) {
  ...
  (config?.subagents_sources || []).forEach((item) => {
...
function matchesRepositorySubagent(
  ...
  if (repo.subagent_id === candidate.id) {
    return true;
  }
...
function getSourceTypeLabel(sourceType: string, t: (...args: any[]) => unknown) {
  switch (sourceType) {
    case 'remote':
      return String(t('subagentsSourceTypeRemote', 'Recommended Source'));
    case 'local_import':
      return String(t('subagentsSourceTypeLocalImport', 'Local Import'));
    case 'mirror':
      return String(t('subagentsSourceTypeMirror', 'Mirror'));
    default:
      return sourceType || '-';
  }
}
...
export function WorkspaceSubagentsPanel({
  ...
  const reloadAll = async () => {
    ...
    const [installedRes, catalogRes, repositoryRes, configRes] = await Promise.allSettled([
      ...
      invoke<ApiResp<CatalogSubagent[]>>('subagents_list_catalog', { model: null }),
  ...
  const handleReinstall = async (subagent: SubagentRecord) => {
    ...
    const repo = repositorySubagents.find((item) =>
      matchesRepositorySubagent(item, {
        source_id: subagent.source_id,
        source_rel_path: subagent.source_rel_path,
        id: subagent.id,
        dir_name: subagent.dir_name,
      }),
    );
    if (!repo) {
      setMessage({
            ...
            return (
              ...
              >
                ...
                <div className="mt-2.5 text-sm leading-none text-muted-foreground">
                  {t('subagentsInstalledCount', 'Installed {{count}} subagents', {
      ...
      {!initialLoadDone ? (
        ...
        <div className="py-12 text-center">
          ...
          <h3 className="mb-2 text-lg font-semibold">
            {t('noInstalledSubagentsForModel', '该模型下暂无已安装 Subagents')}
          </h3>
        ...
        <>
          ...
          {visibleCatalog.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <h3 className="mb-2 text-lg font-semibold">
                {t('noRecommendedSubagents', '当前没有可推荐的 Subagents')}
              </h3>
              <p className="text-muted-foreground">
                {t('noRecommendedSubagentsDesc', '请检查 Subagents 源配置，然后同步后重试。')}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <>
          ...
          {visibleRepository.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {t('subagentsRepositoryEmptyDesc', '请先在左侧 Subagents 页面同步来源或导入仓库。')}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {modelTabs.map((tab) => {
                  ...
                  return (
                    <button
                      key={`workspace-subagents-install-${tab.id}`}
                      type="button"
                      disabled={!allowed}
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          {!allowed
                            ? t('subagentsInstallUnavailableForModel', 'This subagent is not available for the selected model.')
                            : selected
                            ? t('selected', 'Selected')
