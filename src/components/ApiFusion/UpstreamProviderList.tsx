...
export function UpstreamProviderList({
  ...
  return (
    <section className="space-y-4" data-testid="api-fusion-providers">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          onClick={onAdd}
          disabled={busy}
          className="inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2 text-sm font-medium text-primary-foreground shadow-sm transition hover:bg-primary/90 disabled:opacity-50"
        >
      ...
      {providers.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed bg-card/50 px-6 py-12 text-center">
          ...
          <button
            ...
            onClick={onAdd}
            disabled={busy}
            className="mt-4 inline-flex items-center gap-2 rounded-lg border bg-background px-3.5 py-1.5 text-xs font-medium shadow-sm transition hover:bg-muted"
          >
          ...
          {providers.map((provider) => {
            ...
            return (
              <div
                ...
                className={`group relative flex flex-col justify-between rounded-2xl border bg-card p-4 shadow-sm transition-all hover:border-primary/40 hover:shadow-md ${
              ...
              >
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <button
                        ...
                        className="truncate text-left text-base font-semibold hover:text-primary transition-colors block w-full"
                        title={provider.name}
                      >
                        {provider.name}
                      </button>
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        ...
                        {provider.default_model ? (
                          <span
                            className="inline-flex max-w-[160px] truncate rounded-md bg-muted/60 px-1.5 py-0.5 font-mono text-[11px] text-muted-foreground"
                  ...
                  {provider.auto_disabled ? (
                    <div
                      className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 p-2.5 text-xs text-amber-700 dark:text-amber-400"
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={busy}
                        className="inline-flex items-center gap-1 rounded-md border border-amber-500/40 bg-background px-2 py-1 text-[11px] font-medium shadow-sm transition hover:bg-amber-500/20 disabled:opacity-50"
                      >
