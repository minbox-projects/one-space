...
import { type FusionUpstreamProvider } from "@/lib/apiFusion";
...
describe("UpstreamProviderList 状态展示与过滤", () => {
  ...
  it("已启用与已禁用服务商卡片分别使用绿色与红色徽章展示状态", () => {
    ...
    // 已启用状态徽章包含绿色类名
    const enabledBadge = screen.getByTestId("api-fusion-status-badge-p-enabled");
    ...
    // 已禁用状态徽章包含红色类名
    const disabledBadge = screen.getByTestId("api-fusion-status-badge-p-disabled");
    ...
    // 自动熔断服务商包含琥珀色徽章
    const autoBadge = screen.getByTestId("api-fusion-status-badge-p-auto");
  ...
  it("过滤后无匹配项时显示微空态并通过重置按钮恢复全部", async () => {
    ...
    expect(screen.getByTestId("api-fusion-providers-filter-empty")).toBeInTheDocument();
    ...
    // 恢复显示全部
    expect(screen.getByText("Active Service")).toBeInTheDocument();
    expect(screen.queryByTestId("api-fusion-providers-filter-empty")).not.toBeInTheDocument();
  });
  ...
  it("中文环境下状态徽章与分段过滤器展示中文标签", async () => {
    ...
    expect(screen.getByTestId("api-fusion-status-badge-p1")).toHaveTextContent("已启用");
    expect(screen.getByTestId("api-fusion-status-badge-p2")).toHaveTextContent("已禁用");
  });
});
