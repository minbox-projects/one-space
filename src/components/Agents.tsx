...
export function Agents({ isVisible = false }: { isVisible?: boolean }) {
  ...
  const [testPrompt, setTestPrompt] = useState('Summarize the current release risks and key action items.');
  ...
  useEffect(() => {
    ...
    setDraft(selected ? { ...selected, tool_policy: { ...selected.tool_policy } } : null);
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await confirmDialog(
      t('assistantDeleteAgentConfirm', 'Delete this agent definition? Existing schedules will need manual reassignment.'),
      {
  ...
  const handleTestRun = async () => {
    ...
    appWindow.setActiveTab?.('ai-assistant');
  };
      ...
      <div className="grid h-full gap-6 xl:grid-cols-[320px,minmax(0,1fr)]">
        <div className="flex min-h-0 flex-col rounded-2xl border bg-card">
          <div className="border-b px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-base font-semibold">{t('agents', '智能体')}</div>
                <div className="text-xs text-muted-foreground">
                  {t('assistantAgentsDesc', 'Design reusable agent definitions with stable model and tool defaults.')}
                </div>
              </div>
              <button
                type="button"
                ...
                {filteredAgents.map((agent) => (
                  <button
                    ...
                    className={`w-full rounded-xl border px-3 py-3 text-left transition-colors ${
                      draft?.id === agent.id ? 'border-primary bg-primary/5' : 'hover:bg-muted/30'
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 line-clamp-2 text-xs text-muted-foreground">
                          {agent.description || agent.output_contract || agent.system_prompt}
                        </div>
                      </div>
                    </div>
                  </button>
        ...
        <div className="min-h-0 rounded-2xl border bg-card">
          <div className="border-b px-6 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-lg font-semibold">{draft?.name || t('assistantEmptyAgent', '选择一个 Agent')}</div>
                <div className="text-sm text-muted-foreground">
                  {t('assistantAgentEditorDesc', 'Bind prompt, output contract, and model defaults in one focused editor.')}
                </div>
              </div>
            ...
            {draft ? (
              <div className="space-y-6">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <label className="space-y-2">
                    <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t('description', 'Description')}</span>
                    <input
                      value={draft.description}
                      onChange={(e) => setDraftPatch({ description: e.target.value })}
                      className="w-full rounded-lg border bg-background px-3 py-2 text-sm"
                    />
                  </label>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <label className="space-y-2">
                    <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t('assistantDefaultAgentProfile', '默认 Agent 模型')}</span>
                    <select
                      value={draft.default_model_profile_id || ''}
                      onChange={(e) => setDraftPatch({ default_model_profile_id: e.target.value || null })}
                      className="w-full rounded-lg border bg-background px-3 py-2 text-sm"
                    >
                  ...
                  <label className="space-y-2">
                    <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t('assistantDefaultSummaryProfile', '轻量模型')}</span>
                    <select
                      value={draft.light_model_profile_id || ''}
                      onChange={(e) => setDraftPatch({ light_model_profile_id: e.target.value || null })}
                      className="w-full rounded-lg border bg-background px-3 py-2 text-sm"
                    >
                      <option value="">{t('selectModel', 'Select model')}</option>
                      {(settings?.profiles || []).map((profile) => (
                ...
                <label className="space-y-2 block">
                  <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t('systemPrompt', 'System Prompt')}</span>
                  <textarea
                    value={draft.system_prompt}
                    onChange={(e) => setDraftPatch({ system_prompt: e.target.value })}
                    className="min-h-[180px] w-full rounded-xl border bg-background px-3 py-3 text-sm leading-6"
                  />
                </label>

                <label className="space-y-2 block">
                  <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{t('assistantOutputContract', '输出契约')}</span>
                  <input
                    value={draft.output_contract}
                    onChange={(e) => setDraftPatch({ output_contract: e.target.value })}
                    className="w-full rounded-lg border bg-background px-3 py-2 text-sm"
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {[
                    ...
                    <label key={key} className="flex items-center justify-between rounded-xl border bg-muted/10 px-4 py-3">
                      <span className="text-sm font-medium">{label}</span>
                      <input
                        type="checkbox"
                        checked={draft.tool_policy[key as keyof typeof draft.tool_policy]}
                        onChange={(e) =>
                          setDraft((current) =>
                            current
                              ? {
              ...
              <div className="text-sm text-muted-foreground">{t('assistantSelectAgentHint', '从左侧选择一个 Agent，或新建一个。')}</div>
            )}
          </div>
        </div>
      </div>
