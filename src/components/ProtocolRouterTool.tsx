...
type RouteConnectionStatus = "connected" | "flaky" | "failed" | "inactive" | "disabled";
type ApiResp<T> = { ok: boolean; data: T; message?: string; code?: string };
type ServiceProvidersState = { providers: Array<Record<string, unknown>> };
...
function copyKey(kind: "endpoint" | "upstream", routeId: string) {
  return `${kind}:${routeId}`;
}
...
function unwrapApiResp<T>(response: ApiResp<T>, fallbackMessage: string): T {
  if (response?.ok && response.data !== undefined) {
    return response.data;
  }
  throw new Error(response?.message || response?.code || fallbackMessage);
}
...
function CopyIconButton({
  ...
  return (
    <button
      ...
      className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border bg-card text-muted-foreground transition hover:bg-muted"
      title={label}
      aria-label={label}
    >
...
export function ProtocolRouterTool({ isVisible = true }: { isVisible?: boolean }) {
  ...
  const routeViewModels = useMemo<ProtocolRouterRouteViewModel[]>(() => {
    ...
    return config.routes.map((route) => ({
      ...
      ...(() => {
        ...
        return {
          ...summary,
          status: route.enabled ? summary.status : ("disabled" as RouteConnectionStatus),
        };
      })(),
    }));
  ...
  const toggleRouteEnabled = async (route: ProtocolRoute, enabled: boolean) => {
    ...
    setMessage({ type: "", text: "" });
    try {
      const response = await invoke<ApiResp<ServiceProvidersState>>("service_providers_list");
      const providerState = unwrapApiResp(
        response,
        t("protocolRouterProviderLoadFailed", "Failed to load service providers."),
      );
      const provider = providerState.providers.find((candidate) => candidate.id === route.claude_provider_id);
      if (!provider) {
        throw new Error(t("protocolRouterRouteProviderMissing", "Route source provider was not found."));
      }
      unwrapApiResp(
        await invoke<ApiResp<Record<string, unknown>>>("service_providers_upsert", {
          provider: {
            ...provider,
            is_enabled: enabled,
          },
        }),
        t("protocolRouterProviderSaveFailed", "Failed to save service provider."),
      );
      await emit("protocol-router-status-update").catch(() => {});
      await load();
    } catch (err) {
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex min-w-0 flex-1 items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="truncate text-sm font-semibold">{item.route.name}</h3>
                        <div className="truncate text-xs text-muted-foreground">{item.route.claude_provider_name}</div>
                      </div>
                    </div>
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 rounded-full border bg-muted/20 px-2.5 py-1">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => void toggleRouteEnabled(item.route, checked)}
                          disabled={togglingRouteId === item.route.id}
                        />
                      </div>
                      <button
                        type="button"
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="grid gap-2 md:grid-cols-[116px_minmax(0,1fr)_32px] md:items-center">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("endpoint", "Endpoint")}</div>
                      <div className="min-w-0 truncate font-mono text-xs text-foreground" title={endpoint}>
                        {endpoint}
                      </div>
                      <CopyIconButton
                    ...
                    <div className="grid gap-2 md:grid-cols-[116px_minmax(0,1fr)_32px] md:items-center">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("protocolRouterUpstreamApiBaseUrl", "Upstream API Base URL")}</div>
                      <div className="min-w-0 truncate font-mono text-xs text-foreground" title={upstreamBaseUrl}>
                        {upstreamBaseUrl}
                      </div>
                      {item.route.base_url ? (
                        <CopyIconButton
                    ...
                    <div className="grid min-w-0 gap-3 border-t pt-3 sm:grid-cols-4">
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("wireApi", "Wire API Format")}</div>
                      ...
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("protocolRouterLastLatency", "Last latency")}</div>
                        <div className="mt-1 truncate text-xs font-medium">{item.lastLatencyMs === null ? "--" : `${item.lastLatencyMs}ms`}</div>
                      </div>
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("calls", "Calls")}</div>
                        <div className="mt-1 truncate text-xs font-medium">{item.callCount.toLocaleString()}</div>
                      </div>
                      <div className="min-w-0">
                        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("tokens", "Tokens")}</div>
                        <div className="mt-1 truncate text-xs font-medium">{item.totalTokens.toLocaleString()}</div>
                      </div>
                    </div>
                  </div>

                  <div className="mt-3 min-w-0">
                    <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t("protocolRouterUpstreamModelMappings", "Upstream Model Mappings")}</div>
                    {item.route.mappings.length > 0 ? (
                      <div className="mt-1 flex min-w-0 gap-1.5 overflow-hidden">
                        {item.route.mappings.map((mapping) => {
                          const label = `${mapping.claude_model} -> ${mapping.upstream_model}`;
                          return (
                            <span
                              key={`${item.route.id}-mapping-tag-${mapping.claude_model}-${mapping.upstream_model}`}
                      ...
                      <div className="mt-1 truncate text-xs text-muted-foreground">
                        {t("protocolRouterNoUpstreamModelMappings", "No upstream model mappings")}
                      </div>
                    )}
                  </div>

                  <div className="mt-3 flex min-w-0 items-center justify-between gap-4 border-t pt-3 text-xs text-muted-foreground">
                    ...
                    <div className="min-w-0">
                      {item.errorSummary ? (
                        <span className="inline-flex max-w-full items-center justify-end gap-1.5 text-rose-700">
