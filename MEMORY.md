...
- 架构、模块边界与归属、公共协议或 schema、跨领域标准、工作流或 agent 规则、难以回退的技术选型发生变更时，不再新增 ADR；决策原因与生命周期记录到 Agent Notes（`.ai-workflow/notes/`，以 `.ai-workflow/notes/README.md` 为格式与治理唯一来源）。本文件记录当前标准（怎么做），与 Notes 不一致即为缺陷，须在同一变更内一起更新。
