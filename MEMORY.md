...
- 上游服务商保留 `protocol` 字段（`chat_completions` 默认 / `responses`）作为继承来源；每条模型映射可另行声明 `protocol`，缺省或 `null` 即继承所属服务商协议；映射还可选填 `display_name`（`Option<String>`，缺省省略序列化字段），作为网关为该本地模型展示的名称，未填写时回退到远端模型名；映射新增 `reasoning_efforts: Vec<String>`（serde 默认、空时省略序列化）保存该模型的官方推理强度档位，由服务商模板创建与同步维护、在服务商详情编辑，绝不写入终端同步配置；每条映射新增持久化 `enabled`（`#[serde(default = "default_true")]`，字段缺省（不出现）视为启用、始终序列化，旧 `api_gateway.json` 无需迁移），禁用映射不参与候选、不成为命中、其 `upstream_model` 绝不参与转发，请求只命中某服务商的禁用映射时该服务商不得回退 `default_model`（等价于未命中），完全未命中任何映射仍按既有规则在协议一致时回退 `default_model`；映射级 `enabled` 与服务商级 `enabled` / `auto_disabled` 相互独立，手动禁用/重新启用服务商（`set_user_enabled` / `manual_reenable`）绝不修改任何映射的启用状态，被用户禁用的映射只能由对该映射的显式操作恢复。终端同步写入的模型清单只取服务商 `enabled && !auto_disabled` 且映射 `enabled` 的行，`opencode` 以映射的 `local_model` 为键写入 `tool_config.models`（值中的名称取 `display_name`，缺省回退远端模型名），`codex` 的 `model` 取首个非空启用映射 `local_model`，没有任何启用映射时才回退首个非空 `default_model`，仍为空则省略该键。候选选择依据映射行协议（缺省继承服务商协议）能否服务入站协议，不再用服务商 `protocol` 字段硬过滤；命中映射但协议不一致的服务商不参与本次请求且不回退 `default_model`，未命中映射才回退 `default_model` 并要求服务商协议一致。`/v1/models` 与前端聚合视图（`aggregateModels` / `AggregatedModelsDialog`）同样只包含服务商 `enabled && !auto_disabled` 且映射 `enabled` 的本地模型；前端 `ProviderDetailDialog` 为每个映射行提供启用开关，禁用行以 `data-disabled="true"` 与弱化样式直接可见，`resolveMappingPreview` 在只命中禁用映射时返回 `null`。「模型列表」页签紧随「上游服务商」并以 `aggregateModels` 的同一行集（服务商 `enabled && !auto_disabled` 且映射启用的本地模型，不再包含默认模型条目）为唯一来源，一个本地模型一行、三列（模型 ID、模型名称、上游来源）；模型名称由 `resolveAggregatedModelName` 按聚合顺序解析，取首个 trim 后非空的映射 `display_name`，否则取首个映射来源的 `upstream_model`；上游列按聚合顺序显示「服务商名 + 上游模型 ID」；页签徽标数与聚合模型指标卡同源（均为 `aggregateModels(config.providers).length`），搜索框仅按模型 ID 与解析后名称做 trim 后大小写不敏感子串匹配（空或全空白显示全部），绝不匹配上游服务商名或上游模型 ID；面板常驻挂载并以 `hidden` 切换以保留查询，聚合为空显示空态、非空查询无匹配显示无匹配态。中继不做请求体转换，调用方协议需与服务商或映射行协议一致；`/v1/messages`（Anthropic）不受支持属已知边界。
- 「服务商模板」是内置官方模型目录（中文「服务商模板」，英文概念 `Provider Template`、区块标题 `Provider Templates`；既有「上游服务商 / Upstream providers」称谓保持不变）：内置快照 `src-tauri/src/api_gateway/provider_templates.json` 由 `templates.rs` 以 `include_str!` 解析为 `ProviderTemplate` / `ProviderTemplateModel`，`opencode-zen` 的名称、`base_url` `https://opencode.ai/zen/v1` 与全部模型字段跟随 `https://models.dev/api.json` 的 `opencode` 条目，`commandcode` 的模型清单跟随公开 `https://api.commandcode.ai/provider/v1/models`、其价格/峰谷/推理强度为应用内整理；只收录网关可服务的 `chat_completions` / `responses` 模型，仅支持 `/messages` 的 CommandCode 模型绝不进入模板。`api_gateway_provider_templates` 按快照顺序返回两个模板及 `synced_at`、`source`、`from_snapshot`，已持久化同步结果优先于快照，离线时完整回退内置快照。用户手动触发 `api_gateway_sync_provider_template`（`reqwest`，15 秒超时、不带凭据）：公开源字段覆盖、源未提供的字段保留模板当前值（内置快照或上次同步结果）；网络失败、非 2xx、非 JSON、结构非法（缺模型数组或条目缺标识）与空模型集为致命错误，返回含来源与原因的可操作错误且绝不写盘，非法数字按缺失处理、星期非致命规范化。`GatewayConfig.provider_templates: Vec<ProviderTemplateState>` 持久化模板状态（含模板、`synced_at`、`source`），全部 serde 默认，旧 `api_gateway.json` 免迁移可读。
- 模板绑定与维护：`GatewayUpstreamProvider` 新增 `template_id: Option<String>` 与 `ignored_models: Vec<String>`，同步只增量下发、绝不整体替换派生服务商：仅当字段仍等于上一版模板值时才更新（服务商 `name`/`base_url`/`protocol`，映射 `display_name`/`protocol`/`reasoning_efforts`，provider-scoped 价格行四档价与 `off_peaks` 且旧 `off_peak` 为空），映射 `enabled` 与 `local_model` 绝不被改动；官方新模型自动补齐映射与 provider-scoped 价格行（除非在 `ignored_models` 中），官方下架模型保留在服务商并由 `isMappingDeprecated` 标记 deprecated、不删除也不阻断转发与聚合；删除的模型记入 `ignored_models`（仅一次）以防后续同步复活，手动创建的（无 `template_id`）服务商完全不受同步影响；整次同步在配置克隆上暂存并只经一次加密 `write_config` 原子落盘，失败则配置保持原状；写入边界仅为模板状态、派生服务商字段/映射与 provider-scoped 价格行，绝不触碰本地 Key、`terminal_syncs`、转发运行状态或历史用量记录。`api_gateway_create_provider_from_template(template_id, name, base_url, protocol, api_key)` 拒绝空 Key 且不写入，名称/`base_url` 为空回退模板值、协议以参数为准，新服务商携带 `template_id`、`default_model` 为空，并携带每个可服务模型的启用映射（`local_model` = `upstream_model`、官方显示名、生效协议、推理强度）与每模型一条 provider-scoped 价格行（四档价与 `off_peaks`）；`api_gateway_delete_provider_model` 删除映射并在绑定服务商上写忽略记录、删价格行（手动服务商只删映射），`api_gateway_restore_provider_model` 只恢复被忽略模型并按模板当前数据重建映射与价格行，模板已无该模型时返回可操作错误且不写入。
