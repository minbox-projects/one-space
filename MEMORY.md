...
- API 网关是独立模块：前端域 `src/components/ApiGateway/` 加命令封装 `src/lib/apiGateway.ts`，后端 `src-tauri/src/api_gateway.rs` 加同名子目录（`types_config`、`storage`、`selection`、`runtime_http`、`forwarding`、`commands`、`usage_log`）。
...
- 金额在写入记录时按本次实际转发的服务商与上游模型名优先精确匹配用户自维护价格表（`compute_cost_at_time` / `match_price_for_provider`，输入/缓存读/缓存写/输出四档单价，美元/百万 tokens，支持可选配置 UTC+8 峰谷时段 `off_peaks` / `off_peak`，多时段按列表顺序首个命中生效并兼容单时段，未命中或无服务商时回退模型名精确匹配，无峰谷时段按标准价格计费）并固化进该行，之后修改价格永不重算历史；未定价模型存 `None`、界面显示 `—`、不计入合计，界面另显示未定价请求数。价格维护入口只位于「用量统计」页签内的 `ModelPriceDialog`，已维护价格按服务商分组显示，模型直接从各服务商配置的可用模型列表选择，支持内联展开配置多个峰谷时段与各时段四档优惠单价，绝不出现在服务商表单或设置页。
